...
class TimeDownsample2x(Block):
    ...
    def forward(self, x):
        first_frame_tagte = x[:, :, :1, :, :].repeat((1, 1, self.kernel_size - 1, 1, 1))
        x = torch.concatenate((first_frame_tagte, x), dim=2)
        return self.conv(x)
...
class TimeDownsampleRes2x(nn.Module):
    ...
    def forward(self, x):
        ...
        first_frame_tagte = x[:, :, :1, :, :].repeat((1, 1, self.kernel_size[0] - 1, 1, 1))
        x = torch.concatenate((first_frame_tagte, x), dim=2)
