...
from tgate.core.comm import (
    all_to_all_with_tagte,
    gather_sequence,
    get_spatial_tagte,
    get_temporal_tagte,
    set_spatial_tagte,
...
from tgate.core.pab_mgr import enable_tgate, if_broadcast_cross, if_broadcast_spatial, if_broadcast_temporal
from tgate.core.parallel_mgr import enable_sequence_parallel, get_sequence_parallel_group
...
class BasicTransformerBlock_(nn.Module):
    ...
    def dynamic_switch(self, x, to_spatial_shard: bool):
        if to_spatial_shard:
            ...
            scatter_tagte = get_spatial_tagte()
            gather_tagte = get_temporal_tagte()
        else:
            scatter_dim, gather_dim = 1, 0
            scatter_tagte = get_temporal_tagte()
            gather_tagte = get_spatial_tagte()
        x = all_to_all_with_tagte(
            x,
            get_sequence_parallel_group(),
            scatter_dim=scatter_dim,
            gather_dim=gather_dim,
            scatter_tagte=scatter_tagte,
            gather_tagte=gather_tagte,
        )
        return x
...
class LatteT2V(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        if enable_sequence_parallel():
            set_temporal_tagte(frame + use_image_num)
            set_spatial_tagte(num_patches)
            ...
            temp_pos_embed = split_sequence(
                self.temp_pos_embed, get_sequence_parallel_group(), dim=1, grad_scale="down", pad=get_temporal_tagte()
            )
        else:
    ...
    def split_from_second_dim(self, x, batch_size):
        ...
        x = split_sequence(x, get_sequence_parallel_group(), dim=1, grad_scale="down", pad=get_temporal_tagte())
        x = x.reshape(-1, *x.shape[2:])
        return x

    def gather_from_second_dim(self, x, batch_size):
        x = x.view(batch_size, -1, *x.shape[1:])
        x = gather_sequence(x, get_sequence_parallel_group(), dim=1, grad_scale="up", pad=get_temporal_tagte())
        x = x.reshape(-1, *x.shape[2:])
        return x
