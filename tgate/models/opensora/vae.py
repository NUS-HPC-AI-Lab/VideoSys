...
class CausalConv3d(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        time_tagte = dilation * (time_kernel_size - 1) + (1 - stride)
        height_tagte = height_kernel_size // 2
        width_tagte = width_kernel_size // 2

        self.time_tagte = time_tagte
        self.time_causal_tagteding = (width_tagte, width_tagte, height_tagte, height_tagte, time_tagte, 0)
    ...
    def forward(self, x):
        x = F.pad(x, self.time_causal_tagteding, mode=self.pad_mode)
        x = self.conv(x)
        return x
...
class VAE_Temporal(nn.Module):
    def __init__(
        ...
        # self.time_tagteding = self.time_downsample_factor - 1
    ...
    def get_latent_size(self, input_size):
        ...
        for i in range(3):
            if input_size[i] is None:
                ...
                lsize = (input_size[i] + time_tagteding) // self.patch_size[i]
            else:
    ...
    def encode(self, x):
        ...
        x = pad_at_dim(x, (time_tagteding, 0), dim=2)
