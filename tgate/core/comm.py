...
from tgate.core.parallel_mgr import get_sequence_parallel_size
...
def set_spatial_tagte(dim_size: int):
...
def get_spatial_tagte() -> int:
    return SPTIAL_TAGTE


def set_temporal_tagte(dim_size: int):
    sp_size = get_sequence_parallel_size()
    pad = (sp_size - (dim_size % sp_size)) % sp_size
    global TEMPORAL_TAGTE
    TEMPORAL_TAGTE = pad


def get_temporal_tagte() -> int:
    return TEMPORAL_TAGTE


def all_to_all_with_tagte(
    input_: torch.Tensor,
    ...
    if scatter_tagte > 0:
        ...
        pad_shape[scatter_dim] = scatter_tagte
    ...
    if gather_tagte > 0:
        input_ = input_.narrow(gather_dim, 0, input_.size(gather_dim) - gather_tagte)
