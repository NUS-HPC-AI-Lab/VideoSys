...
from tgate.core.comm import model_sharding
...
def remove_tagteding(tensor: torch.Tensor, original_shape: Tuple) -> torch.Tensor:
...
def model_gathering(model: torch.nn.Module, model_shape_dict: dict):
    ...
    for name, param in model.named_parameters():
        ...
        if global_rank == 0:
            ...
            param.data = remove_tagteding(all_params, model_shape_dict[name]).view(model_shape_dict[name])
    dist.barrier()
