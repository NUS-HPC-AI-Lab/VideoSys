...
from opendit.models.opensora_plan import LatteT2V_mse, VideoGenPipeline_mse, ae_stride_config, getae_wrapper
...
def main(args):
    ...
    # Load model:
    transformer_model = LatteT2V_mse.from_pretrained(
        args.model_path, subfolder=args.version, cache_dir=args.cache_dir, torch_dtype=torch.float16
    ).to(device)
    # transformer_model = LatteT2V_mse.from_pretrained(args.model_path, low_cpu_mem_usage=False, device_map=None, torch_dtype=torch.float16).to(device)
