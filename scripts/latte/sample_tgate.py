...
def run_base():
    ...
    pipeline.save_video(video, f"./outputs/latte_tgate_base_{prompt}.mp4")
...
def run_pab():
    ...
    save_path = f"./outputs/latte_tgate_acc_{prompt.replace(' ', '_')}_spatial_{config.tgate_config.spatial_threshold}_cross_{config.tgate_config.cross_threshold}_tgate.mp4"
...
# CUDA_VISIBLE_DEVICES=5 torchrun --standalone --nproc_per_node=1 scripts/latte/sample_tgate.py
