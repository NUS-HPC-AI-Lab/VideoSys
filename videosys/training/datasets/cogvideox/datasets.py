...
class DummyVariableVideoTextDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.preprocessed_data:
            ...
            nf = max(1, num_frames//4)
            self.torch_generator.manual_seed(self.seed + index + self.data_size)
            ret["video"] = torch.randn(16, nf, height // 8, width // 8, generator=self.torch_generator)
            ret["text"] = torch.rand(self.text_max_seq_len, self.text_hidden_size, generator=self.torch_generator)
