...
class AttentionBlock(nn.Module):
    def __init__(
        self, shape, embd_dim, n_head, n_layer, dropout, attn_type, attn_dropout, class_cond_dim, frame_cond_shape
    ):
        super().__init__()
        self.use_frame_cond = frame_cond_shape is not None
    ...
    def forward(self, x, cond, decode_step, decode_idx):
        ...
        if self.use_frame_cond:
            ...
            if self.training:
                h = checkpoint(self.enc_attn, h, cond["frame_cond"], cond["frame_cond"], decode_step, decode_idx)
            else:
                h = self.enc_attn(h, cond["frame_cond"], cond["frame_cond"], decode_step, decode_idx)
...
class SparseAttention(nn.Module):
    ...
    def __init__(
        ...
        self.sparsity_config = StridedSparsityConfig(
            shape=shape, n_head=n_head, causal=causal, block=block, num_local_blocks=num_local_blocks
        )
...
class StridedSparsityConfig(object):
    ...
    def set_local_layout(self, layout):
        ...
        for row in range(0, num_blocks):
            ...
            for col in range(max(0, row - self.num_local_blocks), (row + 1 if self.causal else end)):
                layout[:, row, col] = 1
        return layout
...
class AddBroadcastPosEmbed(nn.Module):
    ...
    def forward(self, x, decode_step=None, decode_idx=None):
        ...
        if decode_step is not None:
            embs = tensor_slice(embs, [0, *decode_idx, 0], [x.shape[0], *(1,) * self.n_dim, x.shape[-1]])
